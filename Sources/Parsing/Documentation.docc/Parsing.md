...
Note that this is using `@resultBuilder` syntax in order to describe from top-to-bottom parsers we want to run. First we parse an integer from the beginning of the input, and then we parse a comma.

Already this can consume a little bit from the beginning of the input:

```swift
// Use a mutable substring to incrementally consume from the beginning
var input = """
1,Blob,true
2,Blob Jr.,false
3,Blob Sr.,true
