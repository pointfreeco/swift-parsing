...
extension Parsers {
  ...
  /// the `pullback` operator, which constructs this type.
  public struct Pullback<Downstream, Input>: Parser where Downstream: Parser {
    ...
    @inlinable
    public func parse(_ input: inout Input) async -> Downstream.Output? {
      await self.downstream.parse(&input[keyPath: self.keyPath])
    }
