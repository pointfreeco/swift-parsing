...
extension Parsers {
  ...
  public struct DoubleParser<Input> {
    @inlinable
    public init() {}
  }
  ...
  public struct FloatParser<Input> {
    @inlinable
    public init() {}
  }
    ...
    public struct Float80Parser<Input> {
      @inlinable
      public init() {}
    }
  #endif
...
extension Parsers.DoubleParser: Parser
where
  Input: Collection,
...
where
  ...
  Input.Element == UTF8.CodeUnit
{
  public func print(_ output: Double) -> Input? {
    .init(String(output).utf8)
  }
}

extension Parsers.FloatParser: Parser
where
  Input: Collection,
  Input.SubSequence == Input,
  Input.Element == UTF8.CodeUnit
...
extension Parsers.FloatParser: Printer
where
  Input: AppendableCollection,
  Input.Element == UTF8.CodeUnit
{
  public func print(_ output: Float) -> Input? {
    .init(String(output).utf8)
  }
}

#if !(os(Windows) || os(Android)) && (arch(i386) || arch(x86_64))
  extension Parsers.Float80Parser: Parser
  where
    Input: Collection,
    Input.SubSequence == Input,
    Input.Element == UTF8.CodeUnit
  ...
  extension Parsers.Float80Parser: Printer
  where
    Input: AppendableCollection,
    Input.Element == UTF8.CodeUnit
  {
    public func print(_ output: Float80) -> Input? {
      .init(String(output).utf8)
    }
  }
#endif
