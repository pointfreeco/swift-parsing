...
///
/// It is possible to seamlessly parse on different abstraction levels of strings. Working on
/// high-level and low-level `String` abstractions each have their pros and cons.
///
/// Parsing low-level abstractions, such as `UTF8View` or a collection of UTF-8 code units, has
/// better performance but at the cost of potentially losing correctness. The most canonical example
/// of this is trying to parse the character "é", which can be represented in code units as `[233]`
/// or `[101, 769]`. If you don't remember to always parse both representations you may have a bug
/// where you accidentally fail your parser when it encounters a code unit sequence you don't
/// support.
///
/// On the other hand, parsing high-level inputs, such as `Substring` or `UnicodeScalarView`, can
/// guarantee correctness, but at the cost of performance. For example, `Substring` handles the
/// complexities of extended grapheme clusters and UTF-8 normalization for you, but traversing
/// strings is slower since its elements are variable width.
///
/// The library gives you the tools that allow you to choose which abstraction level you want to
/// work on, as well as the ability to fluidly move between abstraction levels where it makes sense.
///
...
/// ```
///
/// However, we are incurring the cost of parsing `Substring` for this entire parser, even though
/// only the "San José" case needs that power. We can refactor this parser so that "London" and
/// "New York" are parsed on the `UTF8View` level, since they consist of only ASCII characters, and
/// then parse "San José" as `Substring`:
///
/// ```swift
/// let city = OneOf {
...
/// ```
///
/// The `FromSubstring` parser allows us to temporarily leave the world of parsing UTF-8 and instead
/// work on the higher level `Substring` abstraction, which takes care of normalization of the "é"
/// character.
///
/// If we wanted to be _really_ pedantic we could even parse "San Jos" as UTF-8 and then parse only
/// the "é" character as a substring:
///
/// ```swift
/// let city = OneOf {
///   "London".utf8.map { City.london }
...
/// opaque. To get a human-readable description of the error message you can stringify the error.
/// For example, the following `UInt8` parser fails to parse a string that would cause it to
/// overflow:
///
/// ```swift
/// do {
...
/// restoring the input to the original value when failing. The process of restoring the input to
/// the original value is known as "backtracking". Backtracking can be handy when wanting to try
/// many parsers on the same input, and one usually does this by using the ``OneOf`` parser, which
/// automatically backtracks when one of its parsers fails.
///
/// By not requiring backtracking of each individual parser we can greatly simply the logic of
/// parsers and we can coalesce all backtracking logic into just a single parser, the ``OneOf``
/// parser. If you really need backtracking capabilities then we recommend using the ``OneOf``
/// parser to control backtracking.
///
...
/// In particular, the first integer will get parsed twice, unnecessarily repeating that work. On
/// the other hand, we can factor out the common work of the parser and localize the backtracking
