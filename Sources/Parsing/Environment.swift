...
extension Parsers.EnvironmentKeyWritingParser: Printer
where Upstream: Printer
{
...
//      for index in EnvironmentValues.shared.storage.indices {
        EnvironmentValues.shared[keyPath: self.keyPath] = output
//      }
      return ()
    }
  }
...
extension Parsers.EnvironmentCaptureParser: Printer
where Upstream: Printer
{
  public func print(_ output: Void) -> Upstream.Input? {
    self.upstream.print(EnvironmentValues.shared[keyPath: self.keyPath])
  }
}
