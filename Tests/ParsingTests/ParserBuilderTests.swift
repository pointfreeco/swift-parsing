import CustomDump
import Parsing
import XCTest

final class ParserBuilderTests: XCTestCase {
  func testBuildIfVoid() {
    var parseComma = true
    var parser = Parse {
      "Hello"
      if parseComma {
        ","
      }
      " "
      Prefix { $0 != "!" }
      "!"
    }
    var input = "Hello, world!"[...]
    XCTAssertNoDifference("world", try parser.parse(&input))
    XCTAssertNoDifference(input, ""[...])

    input = "Hello world!"[...]
    XCTAssertThrowsError(try parser.parse(&input)) { error in
      XCTAssertNoDifference(
        """
        error: unexpected input
         --> input:1:6
        1 | Hello world!
          |      ^ expected ","
        """,
        "\(error)"
      )
    }
    XCTAssertNoDifference(input, " world!"[...])

    parseComma = false
    parser = Parse {
      "Hello"
      if parseComma {
        ","
      }
      " "
      Prefix { $0 != "!" }
      "!"
    }
    input = "Hello world!"
    XCTAssertNoDifference("world", try parser.parse(&input))
    XCTAssertNoDifference(input, ""[...])

    input = "Hello, world!"
    XCTAssertThrowsError(try parser.parse(&input)) { error in
      XCTAssertNoDifference(
        """
        error: unexpected input
         --> input:1:6
        1 | Hello, world!
          |      ^ expected " "
        """,
        "\(error)"
      )
    }
    XCTAssertNoDifference(input, ", world!"[...])
  }

  func testBuildIfOutput() throws {
    var parseInt = true
    var parser = Parse {
      if parseInt {
        Int.parser()
        " "
      }
      Rest()
    }
    var input = "42 Blob"[...]
    var (int, string) = try XCTUnwrap(parser.parse(&input))
    XCTAssertNoDifference(42, int)
    XCTAssertNoDifference("Blob", string)
    XCTAssertNoDifference(input, ""[...])

    input = "Blob"
    XCTAssertThrowsError(try parser.parse(&input)) { error in
      XCTAssertNoDifference(
        """
        error: unexpected input
         --> input:1:1
        1 | Blob
          | ^ expected integer
        """,
        "\(error)"
      )
    }
    XCTAssertNoDifference(input, "Blob"[...])

    parseInt = false
    parser = Parse {
      if parseInt {
        Int.parser()
        " "
      }
      Rest()
    }
    input = "Blob"
    (int, string) = try XCTUnwrap(parser.parse(&input))
    XCTAssertNoDifference(nil, int)
    XCTAssertNoDifference("Blob", string)
    XCTAssertNoDifference(input, ""[...])
  }

  func testWrapsCustomErrors() {
    struct MyParser: Parser {
      func parse(_ input: inout Substring) throws {
        struct MyError: LocalizedError {
          var errorDescription: String? {
            "whoops!"
          }
        }
        throw MyError()
      }
    }

    var input = "123 Blob"[...]
    XCTAssertThrowsError(
      try Parse {
        Int.parser()
        MyParser()
      }
      .parse(&input)
    ) { error in
      XCTAssertNoDifference(
        """
        error: whoops!
         --> input:1:4
        1 | 123 Blob
          |    ^
        """,
        "\(error)"
      )
    }
    XCTAssertNoDifference(input, " Blob"[...])

    input = "123 Blob"[...]
    func custom<P>(@ParserBuilder _ build: () -> P) -> P {
      build()
    }
    XCTAssertThrowsError(
      try custom {
        Int.parser()
        MyParser()
      }
      .parse(&input)
    ) { error in
      XCTAssertNoDifference(
        """
        error: whoops!
         --> input:1:4
        1 | 123 Blob
          |    ^
        """,
        "\(error)"
      )
    }
    XCTAssertNoDifference(input, " Blob"[...])
  }
}
